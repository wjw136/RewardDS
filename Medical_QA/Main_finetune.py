...
import time
import numpy as np
from Dataset.medqa_dataset import SingleR_Train_Dataset, SingleR_Train_Dataset_Orpo, collate_fn_Train, collate_fn_Train_orpo
            ...
            if self.bold_train_loader is not None:
                for idx, batch_text in enumerate(self.bold_train_loader):
                    ...
                    if main_verbose and global_step_idx % self.train_log_steps == 0 and global_step_idx != 0:
                        ...
                        print(f"===============================================")
                    if self.use_eval and (((global_step_idx + 1) % self.n_eval_steps == 0) or ((global_step_idx + 1) == len(self.train_dataloader))):
                        train_loss_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_eval", type=str2bool, default="True", help='model name')
    ...
    config['use_eval'] = args.use_eval
    if not args.use_eval:
        config['dev_data_path'] = None
