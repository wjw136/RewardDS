...
from Dataset.medqa_dataset import Dataset_grad, collate_fn_noR, Dataset_optimizer, new_variable_start_tag, new_variable_end_tag
import time
import re
...
def main(args):
    ...
    with torch.no_grad():   
        for idx, batch_data in enumerate(tqdm(test_dataloader)):
            instructions, inputs = batch_data['instructions'], batch_data['questions']

            outputs = model.generate(
                prompts=instructions,
                sampling_params=sampling_params
            )
            for sub_idx, (instruction, input, output) in enumerate(zip(instructions, inputs, outputs)):
