...
class EvaluateDataset_candidate_moment(Dataset):
        ...
        for idx, item in enumerate(self.raw_data_list):
            ...
            self.process_data_list.append(
                {   
                    ...
                    "raw_instruction": item['instruction'],
    ...
    def __getitem__(self, index):
        ...
        return instruction, raw_instruction, entry_point, candidate_samples, candidate_responses
...
def lambda_collate_fn_candidate_moment(batch, tokenizer=None):
    ...
    raw_instruction = [item[1] for item in batch]
    entry_point = [item[2] for item in batch]
    candidate_samples = [item[3] for item in batch]
    candidate_responses = [item[4] for item in batch]
    
    return { 'instruction': instruction, 'raw_instruction': raw_instruction, 'entry_point': entry_point, 'candidate_samples': candidate_samples, "candidate_responses": candidate_responses}
...
def cal_ppl(model, tokenizer, device, instruction, response):
    # 将问题和回答分开，确保回答部分作为目标
    question_text = [
                {
                    "role": "system",
    ...
    new_label_ids = torch.concat([torch.tensor([-100] * question_ids.shape[1]).unsqueeze(0).to(device), target_ids], dim=1)
    # 获取模型的输出
    with torch.no_grad():
        # 模型的输入是问题部分，目标是回答部分
    ...
    for idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        for i in range(len(candidate_samples)):
            ...
            for j in range(len(candidate_responses[i])):
                response_ppl = cal_ppl(ppl_model, tokenizer, device, instructions[i], candidate_responses[i][j])
