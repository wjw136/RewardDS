...
class SingleR_Dataset(Dataset):
    def __init__(self, dataset_dir = "",
                    stimulate_device_id = 0, stimulate_device_num = 10, 
        ...
        if mode == "train":
            ...
            if dev_data_path == "":
                self.raw_data_list = self.raw_data_list[:int(len(self.raw_data_list) * 0.98)]
        elif mode == "dev":
            if dev_data_path != "":
        ...
        for item in self.raw_data_list:
            messages_question = [
                ...
                {
                    "role": "user", "content": item['input'] if 'input' in item else item['question']
                },
            ]
...
#! TextGrad
###############
SYS_MSG_grad="You are a smart language model that evaluates the training sample for the medical question answering task. " + \
    "Based on your understanding of basic and clinical science, medical knowledge, and mechanisms underlying health, disease, patient care, and modes of therapy, give the feedback for training sample. " + \
